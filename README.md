...
```
dbx execute --cluster-name=<name of interactive cluster> --job=lendingclubdb4ml-sample-integration-test
```
...
- To trigger the release pipeline, get the current version from the `lendingclubdb4ml/__init__.py` file and tag the current code version:
```
